...
public class SysInfoService {
	...
	public static List<Map<String, String>> getCpuTotal() {
		Sigar sigar = new Sigar();
		...
		try {
			...
			for (int i = 0; i < infos.length; i++) {// 不管是单块CPU还是多CPU都适用
				...
				datas.put("mhz", info.getMhz() + "");
				datas.put("vendor", info.getVendor() + "");
				datas.put("model", info.getModel() + "");
				datas.put("cacheSize", info.getCacheSize() + "");
				datas.put("totalCores", info.getTotalCores() + "");
				datas.put("totalSockets", info.getTotalSockets() + "");
				datas.put("coresPerSocket", info.getCoresPerSocket() + "");
				dataList.add(datas);
			}
	...
	public static List<Map<String, String>> cpuPerc() {
		...
		for (int i = 0; i < cpuList.length; i++) {
			Map<String, String> dataMap = printCpuPerc(cpuList[i]);
	...
	private static Map<String, String> printCpuPerc(CpuPerc cpu) {
		...
		dataMap.put("Wait", CpuPerc.format(cpu.getWait()));
		dataMap.put("Nice", CpuPerc.format(cpu.getNice()));
		dataMap.put("Idle", CpuPerc.format(cpu.getIdle()));
		dataMap.put("Total", CpuPerc.format(cpu.getCombined()));
		/*
	...
	public static List<Map<String, String>> FileSystemInfo() throws Exception {
		...
		for (int i = 0; i < fslist.length; i++) {
			...
			dataMap.put("DevName", fs.getDevName());
			dataMap.put("SysTypeName", fs.getSysTypeName());
			dataMap.put("TypeName", fs.getTypeName());
			dataMap.put("Type", fs.getType() + "");
			...
			case 2: // TYPE_LOCAL_DISK : 本地硬盘
				dataMap.put("Total", usage.getTotal() / 1024 + "MB");
				dataMap.put("Free", usage.getFree() / 1024 + "MB");
				dataMap.put("Avail", usage.getAvail() / 1024 + "MB");
			...
			dataMap.put("DiskReads", usage.getDiskReads() + "");
			dataMap.put("DiskWrites", usage.getDiskWrites() + "");
			dataList.add(dataMap);
		}
		return dataList;
	}
