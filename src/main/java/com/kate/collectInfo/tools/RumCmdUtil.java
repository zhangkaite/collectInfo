package com.kate.collectInfo.tools;
...
public class RumCmdUtil {
	...
	public static Map<String, Object> startService(String serviceName) throws IOException {
		String[] cmdStr = { "cmd", "/C", " net start " + serviceName };// runAs
																		// /user:Administrator
	...
	public static Map<String, Object> stopService(String serviceName) throws IOException {
		String[] cmdStr = { "cmd", "/C", "net stop " + serviceName };// runAs
																		// /user:Administrator
		logger.info(Arrays.toString(cmdStr));
		try {
			return execCommand(cmdStr);
	...
	public static Map<String, Object> disableService(String serviceName) throws IOException {
		String[] cmdStr = { "cmd", "/C", "sc config " + serviceName + " start= disabled" };// runAs
																							// /user:Administrator
		logger.info(Arrays.toString(cmdStr));
		try {
			return execCommand(cmdStr);
	...
	public static Map<String, Object> enableService(String serviceName) throws IOException {
		String[] cmdStr = { "cmd", "/C", "sc config " + serviceName + " start= auto" };// runAs
																						// /user:Administrator
		logger.info(Arrays.toString(cmdStr));
		try {
			return execCommand(cmdStr);
	...
	public static Map<String, Object> demandService(String serviceName) throws IOException {
		String[] cmdStr = { "cmd", "/C", "sc config " + serviceName + " start= demand" };// runAs
																							// /user:Administrator
		logger.info(Arrays.toString(cmdStr));
		try {
			return execCommand(cmdStr);
	...
	public static Map<String, Object> deleteProcess(String taskname) throws IOException {
		String[] cmdStr = { "cmd", "/C", " wmic process where name='" + taskname + "' delete" };// runAs
																								// /user:Administrator
		try {
			return execCommand(cmdStr);
		} catch (IOException e) {
	 ...
	 * @return
	 * @throws Exception
	 * @see sysTime：表示该时间段内总的CPU时间=CPU处于用户态和内核态CPU时间的总和，即sysTime =kerneTimel +
	 *      userTime（注：这里并不包括idleTime，因为当CPU处于空闲状态时，实在内核模式下运行System Idle
	 *      Process这个进程，所以kernelTime实际上已经包含了idleTime）；
